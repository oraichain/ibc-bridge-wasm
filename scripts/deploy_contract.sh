...
if [ -z $code_id ]
then 
    store_ret=$(oraid tx wasm store $contract_path --from $USER --gas="auto" --gas-adjustment="1.2" --chain-id=$CHAIN_ID -y --keyring-backend test -b block --output json)
    echo $store_ret
...
oraid tx wasm instantiate $code_id "$init" --from $USER --label "$label" --gas auto --gas-adjustment 1.2 --admin $admin --chain-id=$CHAIN_ID -y --keyring-backend test -b block --output json
