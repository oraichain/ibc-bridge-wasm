...
store_ret=$(oraid tx wasm store $contract_path --from $USER --gas="auto" --gas-adjustment="1.2" --chain-id=$CHAIN_ID -y --keyring-backend test --output json -b block)
echo $store_ret
...
oraid tx wasm migrate $contract $code_id $migrate --from $USER --gas auto --gas-adjustment 1.2 --chain-id=$CHAIN_ID -y --keyring-backend test --output json -b block
