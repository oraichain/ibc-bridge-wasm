...
import { OraiswapFactoryClient, OraiswapRouterClient, OraiswapTokenClient, OraiswapPairClient, OraiswapOracleClient } from '@oraichain/oraidex-contracts-sdk';
...
describe.only('IBCModule', () => {
  ...
  let usdtIbcDenom: string = 'tron-testnet0xdac17f958d2ee523a2206206994597c13d831ec7';
  ...
  it.each([
    ...
    [null, ibcTransferAmount, oraiIbcDenom, [], 'cw-ics20-fail-no-pair-mapping-should-not-send-balance-remote-to-local'],
    [
      {
        native_token: {
          denom: ORAI
    ...
    async (assetInfo: AssetInfo, transferAmount: string, transferDenom: string, expectedBalance: Coin[], _name: string) => {
      // create mapping
      if (assetInfo) {
        const pair = {
  ...
  it.each([
    [parseToIbcWasmMemo('', '', ''), ibcTransferAmount, 'empty-memo-should-fallback-to-transfer-to-receiver'],
    [parseToIbcWasmMemo(bobAddress, '', ''), ibcTransferAmount, 'only-receiver-memo-should-fallback-to-transfer-to-receiver'],
    [parseToIbcWasmMemo(bobAddress, oraib2oraichain, ''), ibcTransferAmount, 'receiver-and-channel-memo-should-fallback-to-transfer-to-receiver']
  ...
  describe('cw-ics20-test-single-step-swap-to-tokens', () => {
    ...
    const findWasmEvent = (events: Event[], key: string, value: string) => events.find((event) => event.type === 'wasm' && event.attributes.find((attr) => attr.key === key && attr.value === value));
    beforeEach(async () => {
      assetInfos = [{ native_token: { denom: ORAI } }, { token: { contract_addr: airiToken.contractAddress } }];
      // upload pair & lp token code id
      const { codeId: pairCodeId } = await oraiClient.upload(oraiSenderAddress, readFileSync(oraidexArtifacts.getContractDir('oraiswap_pair')), 'auto');
      const { codeId: lpCodeId } = await oraiClient.upload(oraiSenderAddress, readFileSync(oraidexArtifacts.getContractDir('oraiswap_token')), 'auto');
      lpId = lpCodeId;
      ...
      // deploy oracle addr
      const { contractAddress: oracleAddress } = await oraidexArtifacts.deployContract(oraiClient, oraiSenderAddress, {}, 'oraiswap-oracle', 'oraiswap_oracle');
      ...
      oraiClient.app.bank.setBalance(thirdPairInfo.contract_addr, [coin(initialBalanceAmount, ORAI), coin(initialBalanceAmount, AtomDenom)]);
    });
    ...
    it.each<[string, string, string]>([
      [parseToIbcWasmMemo(bobAddress, '', 'orai'), bobAddress, 'Generic error: Destination channel empty in build ibc msg'],
      [parseToIbcWasmMemo('not-evm-based-nor-cosmos-based', channel, oraiIbcDenom), bobAddress, 'Generic error: The destination info is neither evm nor cosmos based']
    ])('cw-ics20-test-single-step-native-token-swap-operations-to-dest-denom memo %s expected recipient %s', async (memo: string, expectedRecipient: string, expectedIbcErrorMsg: string) => {
      await ics20Contract.updateMappingPair({
        localAssetInfo: {
          native_token: {
            denom: ORAI
      ...
      const transferEvent = result.events.find((event) => event.type === 'transfer' && event.attributes.find((attr) => attr.key === 'recipient' && attr.value === expectedRecipient));
    ...
    it.each([
      [`${bobAddress}`, 'orai1n6fwuamldz6mv5f3qwe9296pudjjemhmkfcgc3', bobAddress, 'Generic error: Destination channel empty in build ibc msg'], // hard-coded usdt address
      [`${bobAddress}`, 'orai18cvw806fj5n7xxz06ak8vjunveeks4zzzn37cu', bobAddress, 'Generic error: Destination channel empty in build ibc msg'] // edge case, dest denom is also airi
    ])('cw-ics20-test-single-step-cw20-token-swap-operations-to-dest-denom memo %s dest denom %s expected recipient %s', async (destReceiver: string, destDenom: string, expectedRecipient: string, expectedIbcErrorMsg: string) => {
      // now send ibc package
    ...
    it('cw-ics20-test-single-step-cw20-FAILED-SWAP_OPS_FAILURE_ID-ack-SUCCESS', async () => {
      ...
      icsPackage.memo = parseToIbcWasmMemo(bobAddress, '', usdtToken.contractAddress);
    ...
    it('cw-ics20-test-single-step-cw20-FAILED-IBC_TRANSFER_NATIVE_ERROR_ID-ack-SUCCESS', async () => {
      ...
      expect(
        result.events.find((ev) => ev.type === 'wasm' && ev.attributes.find((attr) => attr.key === 'action' && attr.value === 'transfer') && ev.attributes.find((attr) => attr.key === 'to' && attr.value === bobAddress))
      ).toBeUndefined();
    });
    ...
    it.each([
    ...
    ])('cw-ics20-test-single-step-has-ibc-msg-dest-fail memo %s dest denom %s expected error', async (destChannel: string, destReceiver: string, destDenom: string) => {
      // now send ibc package
      // icsPackage.memo = `${destChannel}/${destReceiver}:${destDenom}`;
      icsPackage.memo = parseToIbcWasmMemo(destReceiver, destChannel, destDenom);
      // transfer from cosmos to oraichain, should pass
      const result = await cosmosChain.ibc.sendPacketReceive({
        packet: {
          data: toBinary(icsPackage),
      ...
      const ibcEvent = result.events.find((event) => event.type === 'transfer' && event.attributes.find((attr) => attr.key === 'channel'));
      // get swap operation event
      expect(ibcEvent).toBeUndefined();
      const ibcErrorMsg = result.attributes.find((attr) => attr.key === 'ibc_error_msg' && attr.value === 'Generic error: The destination info is neither evm nor cosmos based');
      expect(ibcErrorMsg).not.toBeUndefined();
    });

    it.each([
      [channel, bridgeReceiver, airiIbcDenom] // hard-coded airi
    ])('cw-ics20-test-single-step-has-ibc-msg-dest-receiver-evm-based memo %s dest denom %s expected recipient %s', async (destChannel: string, destReceiver: string, destDenom: string) => {
      // now send ibc package
      // icsPackage.memo = `${destChannel}/${destReceiver}:${destDenom}`;
      icsPackage.memo = parseToIbcWasmMemo(destReceiver, destChannel, destDenom);
    ...
    describe('test-single-step-cosmos-based-ibc-transfer-native', () => {
      ...
      it.each([
      ...
      ])('cw-ics20-test-single-step-has-ibc-msg-dest-receiver-cosmos-based dest channel %s dest denom %s expected recipient %s', async (destChannel: string, destReceiver: string, destDenom: string) => {
        // now send ibc package
        // icsPackage.memo = `${destChannel}/${destReceiver}:${destDenom}`;
        icsPackage.memo = parseToIbcWasmMemo(destReceiver, destChannel, destDenom);
        // transfer from cosmos to oraichain, should pass
        ...
        expect(ibcEvent.attributes.find((attr) => attr.key === 'amount').value).toContain(AtomDenom);
      });
      ...
      it('cw-ics20-test-single-step-ibc-msg-SUCCESS-map-with-fee-denom-orai-and-orai-destination-denom-with-dest-channel-not-matched-with-mapping-pair-should-do-ibctransfer', async () => {
        ...
        const transferEvent = result.events.find((event) => event.type === 'transfer' && event.attributes.find((attr) => attr.key === 'channel'));
        console.log(transferEvent);
        expect(transferEvent.attributes.filter((attr) => attr.key === 'recipient' && attr.value === bobAddress).length).toBeGreaterThan(0);
        expect(transferEvent.attributes.filter((attr) => attr.key === 'amount' && attr.value.includes(AtomDenom)).length).toBeGreaterThan(0);
        expect(transferEvent.attributes.filter((attr) => attr.key === 'channel' && attr.value === unknownChannel).length).toBeGreaterThan(0);
      });
    });
    ...
    it('cw-ics20-test-single-step-handle_ibc_packet_receive_native_remote_chain-has-relayer-fee-should-be-deducted', async () => {
      ...
      expect(result.attributes.find((attr) => attr.key === 'relayer_fee' && attr.value === relayerFee)).not.toBeUndefined();
    });

    it.each<[string, string]>([
      [parseToIbcWasmMemo(bobAddress, channel, oraiIbcDenom), '20000000'],
      [parseToIbcWasmMemo(bobAddress, '', 'orai'), '10000000']
    ])('cw-ics20-test-single-step-ibc-handle_ibc_packet_receive_native_remote_chain-has-token-fee-should-be-deducted', async (memo, expectedTokenFee) => {
      // setup relayer fee
      await ics20Contract.updateConfig({
        tokenFee: [
          {
      ...
      const hasTokenFee = result.events.filter((event) => event.type === 'wasm' && event.attributes.find((attr) => attr.key === 'to' && attr.value === senderAddress));
    ...
    it.each<[string, string, string]>([
      [parseToIbcWasmMemo(bobAddress, channel, airiIbcDenom), '20000000', '100000'],
      [parseToIbcWasmMemo(bridgeReceiver, channel, airiIbcDenom), '20000000', '200000'], // double deducted when there's an outgoing ibc msg after receiving the packet
      ...
      async (memo, expectedTokenFee, expectedRelayerFee) => {
        ...
        expect(hasRelayerFee).not.toBeUndefined();
        expect(result.attributes.find((attr) => attr.key === 'relayer_fee' && attr.value === expectedRelayerFee)).not.toBeUndefined();

        const hasTokenFee = result.events.find(
          (event) => event.type === 'wasm' && event.attributes.find((attr) => attr.key === 'to' && attr.value === senderAddress) && event.attributes.find((attr) => attr.key === 'amount' && attr.value === expectedTokenFee)
        );
        expect(hasTokenFee).not.toBeUndefined();
        expect(result.attributes.find((attr) => attr.key === 'token_fee' && attr.value === expectedTokenFee)).not.toBeUndefined();
      }
    );

    it.each<[string, string, string, string, string]>([[ibcTransferAmount, ibcTransferAmount, '10000000', '90000000', ibcTransferAmount]])(
      ...
      async (transferAmount, relayerFee, expectedTokenFee, expectedRelayerFee, expectedTotalFee) => {
        ...
        const hasFees = result.events.find(
          (event) => event.type === 'wasm' && event.attributes.find((attr) => attr.key === 'to' && attr.value === senderAddress) && event.attributes.find((attr) => attr.key === 'amount' && attr.value === expectedTotalFee)
        );
