...
describe.only("IBCModule", () => {
  ...
  describe("cw-ics20-test-single-step-swap-to-tokens", () => {
    ...
    it("cw-ics20-test-single-step-ibc-msg-FAILED-map-with-fee-denom-orai-and-orai-destination-denom-with-dest-channel-not-matched-with-mapping-pair-channel-no-relay-channel-should-fail", async () => {
      ...
      expect(
        (result.messages[0].msg as any).wasm.execute.contract_addr
      ).toEqual(ics20Contract.contractAddress);
      expect(
        JSON.parse(
          Buffer.from(
            (result.messages[0].msg as any).wasm.execute.msg,
            "base64"
          ).toString("utf8")
        )
      ).toEqual({
      ...
      expect((result.messages[1].msg as any).ibc.transfer.channel_id).toEqual(
        unknownChannel
      );
      expect((result.messages[1].msg as any).ibc.transfer.to_address).toEqual(
        bobAddress
      );
      expect((result.messages[1].msg as any).ibc.transfer.amount).toEqual({
        denom: ORAI,
        amount: ibcTransferAmount,
      });
    });
